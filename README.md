...
* **取引先・品目の自動登録 (New!):** 取引先名や品目名を指定しておくだけで、freee側のIDを自動検索して紐付けて登録します（オプショナル設定）。
...
### オプショナル設定（取引先・品目の自動登録）

取引先や品目を自動で設定したい場合は、**AWS Secrets Manager の認証情報 JSON（推奨）**、または Lambda 関数の環境変数に以下のキーを追加してください。未設定の場合は付与されずに登録されるため、設定しなくても問題なく動作します（後方互換性対応）。
...
* **v1.4.0** (2026-09-03)
  ...
  * AWS Secrets Manager の JSON（推奨）または環境変数（`PARTNER_NAME`, `ITEM_NAME_URIAGE`, `ITEM_NAME_TESURYO`）に登録済みの名前を指定することで、freee側のIDを自動検索して紐付けて登録。
